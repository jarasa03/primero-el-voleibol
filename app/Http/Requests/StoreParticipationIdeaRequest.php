...
class StoreParticipationIdeaRequest extends FormRequest
{
    ...
    {
        return [
            'response_preference' => ['required', 'string', Rule::in(['identified', 'anonymous'])],
            'name' => [
                'nullable',
                'string',
                'max:120',
                Rule::requiredIf(fn (): bool => $this->input('response_preference') === 'identified'),
                Rule::prohibitedIf(fn (): bool => $this->input('response_preference') === 'anonymous'),
            ],
            'email' => [
                'nullable',
                'email:rfc',
